        ...
        for task in path:
            if task_result.get(task) != None:
                ...
                if published_result[idx].get(task) == None:
                    published_result[idx][task] = dict()
                    ...
                    for header in msg.timing_header:
                        for minfo in header.msg_infos:
                            ...
                            if task_history[0] == path[0]:
                                mid = minfo.msg_id
                                if published_result[idx][task].get(mid) == None:
                                    published_result[idx][task][mid] = False
                                published_result[idx][task][mid] = (header.published or published_result[idx][task][mid])
                        path_result[idx][task].append(mid)
                        # published_result[idx][task].append(header.published)
    
    ### eliminate duplicated message ids....
    for idx, path in enumerate(paths):
        for task in path:
            path_result[idx][task] = list(set(path_result[idx].get(task)))
            path_result[idx][task].sort()
...
def check_drop__(path_results, published_results, paths):
    for p_idx, path in enumerate(paths):
        ...
        for t_idx, curr_task in enumerate(path):
            cnt = 0
            p_cnt = 0
            for idx, val in published_results[p_idx][curr_task].items():
                if val:
                    p_cnt += 1

            for m_idx, mid in enumerate(path_results[p_idx][curr_task]):
                if t_idx < len(path) -1:
                    if mid not in path_results[p_idx][path[t_idx+1]] and published_results[p_idx][curr_task][mid] and (m_idx > 2 and m_idx < len(path_results[p_idx][curr_task]) - 3 ):
                    # if mid not in path_results[p_idx][path[t_idx+1]] and published_results[p_idx][curr_task][mid] :
                        cnt += 1
            ...
            if t_idx < len(path) -1:
                ...
                print("%30s (%s) to %30s (%s) ---> drop count : %6s "%(curr_task.center(30), str(p_cnt).center(6), path[t_idx+1].center(30), str(len(path_results[p_idx][path[t_idx+1]])).center(6), str(cnt).center(6)))
    print("=======================================================================================================================================================================================")
